<div x-data="quizForm()" x-init="init" class="space-y-4">
  <div>
    <x-input-label :value="__('teacher.step_content')" />
    <x-text-area-input
      name="content_quiz"
      id="content_quiz"
      rows="5"
  ...
  <div>
    <x-input-label :value="__('teacher.question')" />
    <x-text-input
      type="text"
      name="question"
      class="w-full"
  ...
  <div>
    <x-input-label :value="__('teacher.input_type')" />
    <x-select-input
      name="quiz_type"
      id="quiz_type"
      x-model="quizType"
      x-bind:disabled="!stepType.startsWith('quiz')">
      <option value="quiz_single">{{ __('teacher.radio_buttons') }}</option>
      <option value="quiz_multiple">{{ __('teacher.checkboxes') }}</option>
    </x-select-input>
  </div>

  <div class="space-y-2">
    <x-input-label :value="__('teacher.options')" />
    ...
    <template x-for="(option, index) in options" :key="index">
      <div
        ...
        <x-text-input
          ...
          :placeholder="__('teacher.option_placeholder')"
          x-bind:disabled="!stepType.startsWith('quiz')"
          required />
        ...
        <button
          ...
          :title="__('teacher.delete_option')">
          <x-icons.remove />
        </button>
      </div>
    </template>
